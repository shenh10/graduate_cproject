function [net, stats] = snn_train(net, imdb, getBatch, varargin)
...
for t=1:params.batchSize:numel(subset)
  ...
  if strcmp(mode, 'train')
    ...
    for i = 1:length(net.layers)
        if ~isempty(net.layers{1,i}.weights)
            net.layers{1,i}.weights{1,2} = zeros(size(net.layers{1,i}.weights{1,2}),'single');
        end
    end
  end
